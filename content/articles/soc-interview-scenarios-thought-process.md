...
2. **What did it do?** The full command line. Obfuscation — base64 strings, concatenation tricks, encoded arguments — indicates someone trying not to be seen. That's separate from what the command actually does, and both matter.
...
2. **Tune around that behavior specifically** — not broadly. Suppress by process lineage, by source account, by time window, or by asset group — whatever is most precise. The goal is to exclude the known-good case without expanding the exclusion further than necessary.
